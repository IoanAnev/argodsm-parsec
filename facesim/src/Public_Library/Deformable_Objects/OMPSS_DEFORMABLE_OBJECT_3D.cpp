...
//#####################################################################
#include "OMPSS_DEFORMABLE_OBJECT_3D.h"
#include "../Forces_And_Torques/BODY_FORCES_3D.h"
#include "../Forces_And_Torques/OMPSS_DIAGONALIZED_FINITE_VOLUME_3D.h"
					...
					const bool use_partially_converged_result, const bool verbose)
{
	...
	for (iteration = 0; iteration < newton_iterations; iteration++)
	{
		...
		if (One_Newton_Step_Toward_Steady_State (cg_tolerance, cg_iterations, time + dt, dX_full, false, 0, iteration == 0) || use_partially_converged_result)
			ARRAY_PARALLEL_OPERATIONS<VECTOR_3D<T>, T, VECTOR_3D<T> >::Add_Scaled_Array_Parallel (dX_full, (T) 1, particles.X.array, *particles.particle_ranges);

        /* 
         * Now that we have obtained the updated positions from the CG, lets compute updated values of the model with them 
         * so we can obtain the Newton Raphson residual 
         */
		...
		external_forces_and_velocities->Add_External_Forces (R_full, time, id_number); //Serial
		external_forces_and_velocities->Zero_Out_Enslaved_Position_Nodes (R_full, time, id_number); //Serial
		LOG::Stop_Time();
		LOG::Time ("AOTSQ - NR loop - Compute residual:");
		supnorm = (T) 0;

		ARRAY_PARALLEL_OPERATIONS<VECTOR_3D<T>, T, VECTOR_3D<T> >::Maximum_Magnitude_Squared_Parallel (R_full, *particles.particle_ranges,supnorm);
        #pragma omp taskwait
