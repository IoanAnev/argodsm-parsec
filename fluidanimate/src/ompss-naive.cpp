...
void AdvanceFrameMT(int threadnum)
{
  ...
  #pragma omp taskwait
  for(int i = 0; i < threadnum; i=i+FACTOR) {
      #pragma omp task label(ClearParticlesMT) priority(0)
      {
      for(int j=0; j<FACTOR; j++)
	      ClearParticlesMT(i+j);
      }
  }
  #pragma omp taskwait
  for(int i = 0; i < threadnum; i=i+FACTOR) {
      #pragma omp task label (RebuildGridMT) priority(0)
      {
      for(int j=0; j<FACTOR; j++)
		RebuildGridMT(i+j);
      }
  ...
  #pragma omp taskwait
  for(int i = 0; i < threadnum; i=i+FACTOR) {
      #pragma omp task label(InitDensitiesAndForcesMT) priority(0) 
      {
      for(int j=0; j<FACTOR; j++)
	      InitDensitiesAndForcesMT(i+j);
      }
  }
  #pragma omp taskwait
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task label(ComputeDensitiesMT) priority(0)
      ComputeDensitiesMT(i);
  }
  #pragma omp taskwait
  for(int i = 0; i < threadnum; i=i+FACTOR) {
      #pragma omp task label (ComputeDensities2MT) priority(42)
      {
      for(int  j=0; j<FACTOR; j++)
	      ComputeDensities2MT(i+j);
      }

  }
  #pragma omp taskwait
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task label(ComputeForcesMT) priority(0)
      ComputeForcesMT(i);
  }
  #pragma omp taskwait
  for(int i = 0; i < threadnum; i=i+FACTOR) {
      #pragma omp task label(ProcessCollisionsMT) priority(42)
      {
      for(int  j=0; j<FACTOR; j++)
      	ProcessCollisionsMT(i+j);
      }
  }
  #pragma omp taskwait
  for(int i = 0; i < threadnum; i=i+FACTOR) {
      #pragma omp task label(AdvanceParticlesMT) priority(0)
      {
      for(int  j=0; j<FACTOR; j++)
        AdvanceParticlesMT(i+j);
      }
  }
  #pragma omp taskwait
#if defined(USE_ImpeneratableWall)
  for(int i = 0; i < threadnum; i=i+FACTOR) {
      #pragma omp task label(ProcessCollisions2MT) priority(0)
      {
	  for(int  j=0; j<FACTOR; j++)
			ProcessCollisions2MT(i+j);
	  }
  }
	#pragma omp taskwait
#endif
