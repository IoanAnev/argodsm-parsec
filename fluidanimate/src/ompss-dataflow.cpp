...
void InitSim(char const *fileName, unsigned int threadnum)
{
  ...
  for(int i = 0; i < numCells; ++i)
  {
    ...
    lock2[i] = new omp_lock_t[n];
    for(int j = 0; j < n; ++j) {
      //pthread_mutex_init(&mutex[i][j], NULL);
      omp_init_lock(&lock[i][j]);
      omp_init_lock(&lock2[i][j]);
    }
  ...
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
      for(int ix = grids[tid].sx; ix < grids[tid].ex; ++ix)
      {
        int index = (iz*ny + iy)*nx + ix;
        Cell *cell = &cells[index];
        //fprintf(stdout, "-1 cell %p\n", *cell);
        int np = cnumPars[index];
        for(int j = 0; j < np; ++j)
        {
          //fprintf(stdout, "0 tid=%d index=%d particle=%d %d pointer d %p\n", tid, index, j, j%PARTICLES_PER_CELL, cell->density);
          //fflush(stdout);
          cell->density[j % PARTICLES_PER_CELL] = 0.0;
          //fprintf(stdout, "1 tid=%d index=%d particle=%d %d pointer a %p\n", tid, index, j, j%PARTICLES_PER_CELL, cell->a);
          //fflush(stdout);
          cell->a[j % PARTICLES_PER_CELL] = externalAcceleration;
          //fprintf(stdout, "2 tid=%d index=%d particle=%d %d pointer n %p n->a %p\n", tid, index, j, j%PARTICLES_PER_CELL, cell->next, cell->next->a);
          //fflush(stdout);
          //move pointer to next cell in list if end of array is reached
          if(j % PARTICLES_PER_CELL == PARTICLES_PER_CELL-1) {
            cell = cell->next;
          }
          //fprintf(stdout, "3 tid=%d index=%d particle=%d %d\n", tid, index, j, j%PARTICLES_PER_CELL);
          //fflush(stdout);
        }
      }
}
      ...
      for(int ix = grids[tid].sx; ix < grids[tid].ex; ++ix)
      {
        ...
        for(int ipar = 0; ipar < np; ++ipar)
        {
          for(int inc = 0; inc < numNeighCells; ++inc)
          {
              ...
              if(&neigh->p[iparNeigh % PARTICLES_PER_CELL] < &cell->p[ipar % PARTICLES_PER_CELL])
              {
                ...
                if(distSq < hSq)
                {
                  ...
                  if( border[index])
                  {
                    //pthread_mutex_lock(&mutex[index][ipar % MUTEXES_PER_CELL]);
		    //printf("111a %d\n", tid);
                    omp_set_lock(       &lock2[index][ipar % MUTEXES_PER_CELL]);
		    //printf("111z %d\n", tid);
                    cell->a[ipar % PARTICLES_PER_CELL] += acc;
		    //printf("222a %d\n", tid);
                    omp_unset_lock(&lock2       [index][ipar % MUTEXES_PER_CELL]);
                    //pthread_mutex_unlock(&mutex[index][ipar % MUTEXES_PER_CELL]);
		    //printf("222z %d\n", tid);
                  }
                  else
		    ...
		    //printf("333a %d\n", tid);
                    //pthread_mutex_lock(&mutex[indexNeigh][iparNeigh % MUTEXES_PER_CELL]);
                    omp_set_lock(&lock2[indexNeigh][iparNeigh % MUTEXES_PER_CELL]);
		    //printf("333z %d\n", tid);
                    neigh->a[iparNeigh % PARTICLES_PER_CELL] -= acc;
		    //printf("444a %d\n", tid);
                    omp_unset_lock(&lock2[indexNeigh][iparNeigh % MUTEXES_PER_CELL]);
                    //pthread_mutex_unlock(&mutex[indexNeigh][iparNeigh % MUTEXES_PER_CELL]);
		    //printf("444z %d\n", tid);
                  }
                  else
...
void AdvanceFrameMT(int threadnum)
{
  ...
  int* z = new int[XDIVS];
  for(int i = 0; i < XDIVS; i++) {
      #pragma omp task label(ClearParticlesMT) out(z[i])
      for (int j=0; j < ZDIVS; ++j) {
        ClearParticlesMT(i*ZDIVS + j);
      }
  }
  //#pragma omp taskwait
  int* y = new int[XDIVS];
  for(int i = 0; i < XDIVS; ++i) {
      #pragma omp task label (RebuildGridMT) in(z[arrange(i-1)], z[i], z[(i+1)%XDIVS])  out(y[i])
      for (int j=0; j < ZDIVS; ++j) {
         RebuildGridMT(i*ZDIVS + j);
      }
  }
  //#pragma omp taskwait
  int* x = new int[XDIVS];
  for(int i = 0; i < XDIVS; ++i) {
      #pragma omp task label(InitDensitiesAndForcesMT) in(y[arrange(i-1)], y[i], y[(i+1)%XDIVS]) out(x[i])
      for (int j=0; j < ZDIVS; ++j) {
         InitDensitiesAndForcesMT(i*ZDIVS + j);
      }
  }
  //#pragma omp taskwait
  int* a = new int[XDIVS];
  for(int i = 0; i < XDIVS; ++i) {
      #pragma omp task label(ComputeDensitiesMT) in(x[arrange(i-1)], x[i], x[(i+1)%XDIVS]) out(a[i])
      for (int j=0; j < ZDIVS; ++j) {
      	ComputeDensitiesMT(i*ZDIVS + j);
      }
  }
  //#pragma omp taskwait
  int* b = new int[XDIVS];
  for(int i = 0; i < XDIVS; ++i) {
     #pragma omp task label(ComputeDensities2MT) in(a[arrange(i-1)], a[i], a[(i+1)%XDIVS]) out(b[i])
      for(int  j=0; j < ZDIVS; j++) {
        ComputeDensities2MT(i*ZDIVS+j);
      }
  }
  //#pragma omp taskwait
  int* c = new int[XDIVS];
  for(int i = 0; i < XDIVS; ++i) {
      #pragma omp task label(ComputeforcesMT) in(b[arrange(i-1)], b[i], b[(i+1)%XDIVS])  out(c[i])
      for (int j=0; j < ZDIVS; ++j) {
        ComputeForcesMT(i*ZDIVS + j);
      }
  }
  //#pragma omp taskwait
  int* d = new int[XDIVS];
  for(int i = 0; i < XDIVS; ++i) {
      #pragma omp task label(ProcessCollisionsMT) in(c[arrange(i-1)], c[i], c[(i+1)%XDIVS]) out(d[i])
      for (int j=0; j < ZDIVS; ++j) {
        ProcessCollisionsMT(i*ZDIVS + j);
      }
  }
  //#pragma omp taskwait
  for(int i = 0; i < XDIVS; ++i) {
      #pragma omp task label(AdvanceParticlesMT) in(d[arrange(i-1)], d[i], d[(i+1)%XDIVS])
      for (int j=0; j < ZDIVS; ++j) {
         AdvanceParticlesMT(i*ZDIVS + j);
      }
  }
  #pragma omp taskwait
}
