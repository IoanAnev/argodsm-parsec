...
//This OmpSs version written by Dimitrios Chasapis and Iulian Brumar - Barcelona Supercomputing Center
...
void AdvanceFrameMT(int ntasks)
{
	...
	for (int i = 0; i < ntasks; ++i) {
		int size = GRIDSIZE(i);
		#pragma omp task out(cnumPars[blocini ; size]) label(ClearParticles)
		ClearParticlesMT(i);
		blocini += size;
	}
	...
	for (int i = 0; i < ntasks; ++i) {
		int size = GRIDSIZE(i);
		#pragma omp task in(cells2[blocini ; size], cnumPars2[blocini ; size]) inout(cnumPars[blocini ; size]) out(cells[blocini ; size]) label(RebuildGrid)
		RebuildGridMT(i);
		blocini += size;
	}
	#pragma omp taskwait
	...
	for (int i = 0; i < ntasks; ++i) {
		int size = GRIDSIZE(i);
		#pragma omp task in(cnumPars[blocini ; size]) inout(cells[blocini ; size]) label(InitDensitiesAndForces)
	...
	for (int i = 0; i < ntasks; ++i) {
		int size = GRIDSIZE(i);
		#pragma omp task in(cnumPars[blocini ; size]) inout(cells[blocini ; size]) label(ComputeDensities)
	...
	for (int i = 0; i < ntasks; ++i) {
		int size = GRIDSIZE(i);
		#pragma omp task inout(cells[blocini ; size]) label(ComputeDensities2)
	...
	for (int i = 0; i < ntasks; ++i) {
		int size = GRIDSIZE(i);
		#pragma omp task in(cnumPars[blocini ; size]) inout(cells[blocini ; size]) label(ComputeForces)
		ComputeForcesMT(i);
		blocini += size;
	}
	//#pragma omp taskwait
	...
	for (int i = 0; i < ntasks; ++i) {
		int size = GRIDSIZE(i);
		#pragma omp task in(cnumPars[blocini ; size]) inout(cells[blocini ; size]) label(ProcessCollisions)
	...
	for (int i = 0; i < ntasks; ++i) {
		int size = GRIDSIZE(i);
		#pragma omp task inout(cells[blocini ; size]) label(AdvanceParticles)
