...
using namespace std;
vector<int> neighbors_per_block;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    #define PROGRAM             1000
    #define END                 0
    #define CLEARPARTICLES      1
    #define REBUILD             2
    #define COMPUTE             3
    #define COMPUTEDENS         4
    #define COMPUTEFORCES       5
    #define PROCESSCOLL         6
    #define PROCESSCOLL2        7
    #define ADVANCEPART         8
    #define COMPUTEPART         9
#endif
...
{
#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,CLEARPARTICLES);
#endif
    //std::cout << "Thread ID is: " << omp_get_thread_num() << std::endl;
...
#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,END);
#endif
}
...
#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,REBUILD);
#endif
    //std::cout << "Thread ID is: " << omp_get_thread_num() << std::endl;
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
...
{
#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,COMPUTE);
#endif
    //std::cout << "Thread ID is: " << omp_get_thread_num() << std::endl;
...
{
#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,COMPUTEDENS);
#endif
    //std::cout << "Thread ID is: " << omp_get_thread_num() << std::endl;
...
{
#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,COMPUTEFORCES);
#endif
    //std::cout << "Thread ID is: " << omp_get_thread_num() << std::endl;
  int neighCells[3*3*3];
...
{
#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,PROCESSCOLL);
#endif
    //std::cout << "Thread ID is: " << omp_get_thread_num() << std::endl;
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
  {
...
{
#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,PROCESSCOLL2);
#endif
    //std::cout << "Thread ID is: " << omp_get_thread_num() << std::endl;
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
  {
...
{
#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,ADVANCEPART);
#endif
    //std::cout << "Thread ID is: " << omp_get_thread_num() << std::endl;
  for(int iz = grids[tid].sz; iz < grids[tid].ez; ++iz)
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
...
void compute_particles(int tid, int *particles) {

#ifdef ENABLE_OMPEXTRAE
    Extrae_eventandcounters(PROGRAM,COMPUTEPART);
#endif
  *particles=0;
...
int compute_neighbors(int tid, std::vector<int>& neighborhood)
{

  //fprintf(stdout, "tid=%d z = [%d,%d], y = [%d,%d], x = [%d,%d] XDIVS=%d ZDIVS=%d\n", tid, grids[tid].sz, grids[tid].ez, grids[tid].sy,  grids[tid].ey, grids[tid].sx,  grids[tid].ex, XDIVS, ZDIVS);
  int numneighbors = 0;
  neighborhood.insert(neighborhood.begin(), tid);
  numneighbors++;

  if(tid % ZDIVS != 0)            {/*fprintf(stdout, "up     neighbor %d\n", tid-1);*/ neighborhood.insert(neighborhood.begin(), tid-1); numneighbors++;}
  if(tid % ZDIVS != ZDIVS - 1)    {/*fprintf(stdout, "bottom neighbor %d\n", tid+1);*/ neighborhood.insert(neighborhood.begin(), tid+1); numneighbors++;}
  if(tid/ZDIVS   != 0)            {/*fprintf(stdout, "left   neighbor %d\n", tid-ZDIVS);*/ neighborhood.insert(neighborhood.begin(), tid-ZDIVS); numneighbors++;}
  if(tid/ZDIVS   != XDIVS - 1)    {/*fprintf(stdout, "right  neighbor %d\n", tid+ZDIVS);*/ neighborhood.insert(neighborhood.begin(), tid+ZDIVS); numneighbors++;}

  //diagonal up left
  if(tid % ZDIVS != 0 && tid/ZDIVS   != 0)         {neighborhood.insert(neighborhood.begin(), tid-ZDIVS-1);numneighbors++;}
  //diagonal up right 
  if(tid % ZDIVS != 0 && tid/ZDIVS   != XDIVS - 1) {neighborhood.insert(neighborhood.begin(), tid+ZDIVS-1);numneighbors++;}
  //diagonal bottom right 
  if(tid % ZDIVS != ZDIVS - 1 && tid/ZDIVS   != XDIVS - 1) {neighborhood.insert(neighborhood.begin(), tid+ZDIVS+1);numneighbors++;}
  //diagonal bottom left
  if(tid % ZDIVS != ZDIVS - 1 && tid/ZDIVS   != 0) {neighborhood.insert(neighborhood.begin(), tid-ZDIVS+1);numneighbors++;}
...
void AdvanceFrameMT(int threadnum, std::vector< vector<int> > neighborhood, std::vector<int> particles, int *dependency_past, int framenum)
{
  //swap src and dest arrays with particles
  std::swap(cells, cells2);
  std::swap(cnumPars, cnumPars2);
  ...
  //#pragma omp taskwait
  int* z = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task depend(in:dependency_past[ii:1]) depend(out:z[i:1]) 
      ClearParticlesMT(i);
  }
  //#pragma omp taskwait
  int* y = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      int numneighbors = neighbors_per_block[i];
      if (numneighbors == 4) {
        #pragma omp task \
        depend(in:z[neighborhood[i][0]:1],z[neighborhood[i][1]:1],z[neighborhood[i][2]:1],z[neighborhood[i][3]:1] ) \
        depend(out:y[i:1] ) 
        RebuildGridMT(i);
      } else if (numneighbors == 6) {
        #pragma omp task \
        depend(in: z[neighborhood[i][0]:1],z[neighborhood[i][1]:1],z[neighborhood[i][2]:1],z[neighborhood[i][3]:1],z[neighborhood[i][4]:1],z[neighborhood[i][5]:1]) \
        depend(out:y[i:1] ) 
        RebuildGridMT(i);
      } else if (numneighbors == 9) {
        #pragma omp task \
        depend(in:z[neighborhood[i][0]:1],z[neighborhood[i][1]:1],z[neighborhood[i][2]:1],z[neighborhood[i][3]:1],z[neighborhood[i][4]:1],z[neighborhood[i][5]:1],z[neighborhood[i][6]:1],z[neighborhood[i][7]:1],z[neighborhood[i][8]:1] ) \
        depend(out:y[i:1] ) 
        RebuildGridMT(i);
      }
  ...
  //#pragma omp taskwait
  int* x = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      int numneighbors = neighbors_per_block[i];
      //std::cout << "Thread " << i << " neighbors: ";
      //for (int j = 0; j < numneighbors; j++) std::cout << neighborhood[i][j] << ", ";
      //std::cout << std::endl;
      if (numneighbors == 4) {
          #pragma omp task \
          depend(in:y[neighborhood[i][0]:1],y[neighborhood[i][1]:1],y[neighborhood[i][2]:1],y[neighborhood[i][3]:1]  ) \
          depend(out:x[i:1] )
          InitDensitiesAndForcesMT(i);
      } else if (numneighbors == 6) {
          #pragma omp task \
          depend(in:y[neighborhood[i][0]:1],y[neighborhood[i][1]:1],y[neighborhood[i][2]:1],y[neighborhood[i][3]:1],y[neighborhood[i][4]:1],y[neighborhood[i][5]:1] ) \
          depend(out:x[i:1] )
          InitDensitiesAndForcesMT(i);
      } else if (numneighbors == 9) {
          #pragma omp task \
          depend(in:y[neighborhood[i][0]:1],y[neighborhood[i][1]:1],y[neighborhood[i][2]:1],y[neighborhood[i][3]:1],y[neighborhood[i][4]:1],y[neighborhood[i][5]:1],y[neighborhood[i][6]:1],y[neighborhood[i][7]:1],y[neighborhood[i][8]:1] ) \
          depend(out:x[i:1] )
          InitDensitiesAndForcesMT(i);
      }
  }
  //#pragma omp taskwait
  int* a = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      int numneighbors = neighbors_per_block[i];
      if (numneighbors == 4) {
          #pragma omp task \
          depend(in:x[neighborhood[i][0]:1],x[neighborhood[i][1]:1],x[neighborhood[i][2]:1],x[neighborhood[i][3]:1]  ) \
          depend(out:a[i:1] ) 
          ComputeDensitiesMT(i);
      } else if (numneighbors == 6) {
          #pragma omp task \
          depend(in:x[neighborhood[i][0]:1],x[neighborhood[i][1]:1],x[neighborhood[i][2]:1],x[neighborhood[i][3]:1],x[neighborhood[i][4]:1],x[neighborhood[i][5]:1] ) \
          depend(out:a[i:1] ) 
          ComputeDensitiesMT(i);
      } else if (numneighbors == 9) {
          #pragma omp task \
          depend(in:x[neighborhood[i][0]:1],x[neighborhood[i][1]:1],x[neighborhood[i][2]:1],x[neighborhood[i][3]:1],x[neighborhood[i][4]:1],x[neighborhood[i][5]:1],x[neighborhood[i][6]:1],x[neighborhood[i][7]:1],x[neighborhood[i][8]:1] ) \
          depend(out:a[i:1] ) 
          ComputeDensitiesMT(i);
      }
  }
  ...
  for(int i = 0; i < threadnum; ++i) {
      ...
      if (numneighbors == 4) {
          #pragma omp task \
          depend(in: a[neighborhood[i][0]:1],a[neighborhood[i][1]:1],a[neighborhood[i][2]:1],a[neighborhood[i][3]:1]) \
          depend(out:b[i:1] ) 
          ComputeDensities2MT(i);
      } else if (numneighbors == 6) {
          #pragma omp task \
          depend(in: a[neighborhood[i][0]:1],a[neighborhood[i][1]:1],a[neighborhood[i][2]:1],a[neighborhood[i][3]:1],a[neighborhood[i][4]:1],a[neighborhood[i][5]:1]) \
          depend(out:b[i:1] ) 
          ComputeDensities2MT(i);
      } else if (numneighbors == 9) {
          #pragma omp task \
          depend(in: a[neighborhood[i][0]:1],a[neighborhood[i][1]:1],a[neighborhood[i][2]:1],a[neighborhood[i][3]:1],a[neighborhood[i][4]:1],a[neighborhood[i][5]:1],a[neighborhood[i][6]:1],a[neighborhood[i][7]:1],a[neighborhood[i][8]:1]) \
          depend(out:b[i:1] ) 
          ComputeDensities2MT(i);
      }
  }
  ...
  for(int i = 0; i < threadnum; ++i) {
      ...
      if (numneighbors == 4) {
          #pragma omp task \
          depend(in: b[neighborhood[i][0]:1],b[neighborhood[i][1]:1],b[neighborhood[i][2]:1],b[neighborhood[i][3]:1]) \
          depend(out:c[i:1]) 
          ComputeForcesMT(i);
      } else if (numneighbors == 6) {
          #pragma omp task \
          depend(in: b[neighborhood[i][0]:1],b[neighborhood[i][1]:1],b[neighborhood[i][2]:1],b[neighborhood[i][3]:1],b[neighborhood[i][4]:1],b[neighborhood[i][5]:1]) \
          depend(out:c[i:1]) 
          ComputeForcesMT(i);
      } else if (numneighbors == 9) {
          #pragma omp task \
          depend(in: b[neighborhood[i][0]:1],b[neighborhood[i][1]:1],b[neighborhood[i][2]:1],b[neighborhood[i][3]:1],b[neighborhood[i][4]:1],b[neighborhood[i][5]:1],b[neighborhood[i][6]:1],b[neighborhood[i][7]:1],b[neighborhood[i][8]:1]) \
          depend(out:c[i:1]) 
          ComputeForcesMT(i);
      }
  }
  //#pragma omp taskwait
  int* d = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      int numneighbors = neighbors_per_block[i];
      if (numneighbors == 4) {
          #pragma omp task \
          depend(in: c[neighborhood[i][0]:1],c[neighborhood[i][1]:1],c[neighborhood[i][2]:1],c[neighborhood[i][3]:1]) \
          depend(out:d[i:1] ) 
          ProcessCollisionsMT(i);
      } else if (numneighbors == 6) {
          #pragma omp task \
          depend(in: c[neighborhood[i][0]:1],c[neighborhood[i][1]:1],c[neighborhood[i][2]:1],c[neighborhood[i][3]:1],c[neighborhood[i][4]:1],c[neighborhood[i][5]:1]) \
          depend(out:d[i:1] ) 
          ProcessCollisionsMT(i);
      } else if (numneighbors == 9) {
          #pragma omp task \
          depend(in: c[neighborhood[i][0]:1],c[neighborhood[i][1]:1],c[neighborhood[i][2]:1],c[neighborhood[i][3]:1],c[neighborhood[i][4]:1],c[neighborhood[i][5]:1],c[neighborhood[i][6]:1],c[neighborhood[i][7]:1],c[neighborhood[i][8]:1]) \
          depend(out:d[i:1] ) 
          ProcessCollisionsMT(i);
      }
  }
  //#pragma omp taskwait
  int* e = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) { //for j=0; j < neighborhood[i].size()
      int numneighbors = neighbors_per_block[i];
       
      if (numneighbors == 4) {
          #pragma omp task \
          depend(in: e[0:1],d[neighborhood[i][0]:1],d[neighborhood[i][1]:1],d[neighborhood[i][2]:1],d[neighborhood[i][3]:1]) \
          depend(out:e[i:1]) 
          AdvanceParticlesMT(i);
      } else if (numneighbors == 6) {
          #pragma omp task \
          depend(in: e[0:1],d[neighborhood[i][0]:1],d[neighborhood[i][1]:1],d[neighborhood[i][2]:1],d[neighborhood[i][3]:1],d[neighborhood[i][4]:1],d[neighborhood[i][5]:1] ) \
          depend(out:e[i:1]) 
          AdvanceParticlesMT(i);
      } else if (numneighbors == 9) {
          #pragma omp task \
          depend(in: e[0:1],d[neighborhood[i][0]:1],d[neighborhood[i][1]:1],d[neighborhood[i][2]:1],d[neighborhood[i][3]:1],d[neighborhood[i][4]:1],d[neighborhood[i][5]:1],d[neighborhood[i][6]:1],d[neighborhood[i][7]:1],d[neighborhood[i][8]:1]) \
          depend(out:e[i:1]) 
          AdvanceParticlesMT(i);
      }
  }
  ...
  #pragma omp task \
  depend(inout:e[0:1]) \
  depend(out:dependency_past[ii+1:1])
  {
...
void *AdvanceFramesMT(int framenum, int threadnum)
{
  ...
  std::vector< vector<int> > neighborhood(threadnum);

  std::vector<int> particles(threadnum);

  

  for(int i = 0; i<threadnum; ++i) {
        neighbors_per_block.push_back(compute_neighbors(i, neighborhood[i] ));
        compute_particles(i, &particles[i] );
  }


  int *dependency_past = (int *)malloc(sizeof(int)*(framenum+1));
  AdvanceFrameMT(threadnum, neighborhood, particles, dependency_past, framenum);
  return NULL;
}
#else
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  __parsec_bench_begin(__parsec_fluidanimate);
    ...
    long double elapsed;
    struct timeval pstop;
    struct timeval pstart;
    long double pelapsed;
...
#pragma omp parallel
{
  ...
  CleanUpSim();
    ...
    elapsed = (long double) 1000000 * (stop.tv_sec - start.tv_sec);
    elapsed += (long double) stop.tv_usec - start.tv_usec;
	std::cout << "Critical execution time: " <<  elapsed/1e6  /*time(NULL) - startt*/ << std::endl;
    gettimeofday(&pstop,NULL);
    pelapsed = (long double) 1000000 * (pstop.tv_sec - pstart.tv_sec);
    pelapsed += (long double) pstop.tv_usec - pstart.tv_usec;
	std::cout << "Total execution time: " <<  pelapsed/1e6  /*time(NULL) - startt*/ << std::endl;
#ifdef ENABLE_PARSEC_HOOKS
  __parsec_bench_end();
#endif
