...
//This OmpSs version written by Marc Casas - Barcelona Supercomputing Center
...
void compute_neighbors(int tid, std::vector<int>& neighborhood)
{
  ...
  if(tid % ZDIVS != 0)            {/*fprintf(stdout, "up     neighbor %d\n", tid-1);*/ neighborhood.insert(neighborhood.begin(), tid-1);}
  if(tid % ZDIVS != ZDIVS - 1)    {/*fprintf(stdout, "bottom neighbor %d\n", tid+1);*/ neighborhood.insert(neighborhood.begin(), tid+1);}
  if(tid/ZDIVS   != 0)            {/*fprintf(stdout, "left   neighbor %d\n", tid-ZDIVS);*/ neighborhood.insert(neighborhood.begin(), tid-ZDIVS);}
  if(tid/ZDIVS   != XDIVS - 1)    {/*fprintf(stdout, "right  neighbor %d\n", tid+ZDIVS);*/ neighborhood.insert(neighborhood.begin(), tid+ZDIVS);}

  //diagonal up left
  if(tid % ZDIVS != 0 && tid/ZDIVS   != 0)         neighborhood.insert(neighborhood.begin(), tid-ZDIVS-1);
  //diagonal up right 
  if(tid % ZDIVS != 0 && tid/ZDIVS   != XDIVS - 1) neighborhood.insert(neighborhood.begin(), tid+ZDIVS-1);
  //diagonal bottom right 
  if(tid % ZDIVS != ZDIVS - 1 && tid/ZDIVS   != XDIVS - 1) neighborhood.insert(neighborhood.begin(), tid+ZDIVS+1);
  //diagonal bottom left
  if(tid % ZDIVS != ZDIVS - 1 && tid/ZDIVS   != 0) neighborhood.insert(neighborhood.begin(), tid-ZDIVS+1);
...
void AdvanceFrameMT(int threadnum, std::vector< vector<int> > neighborhood, std::vector<int> particles, int *dependency_past, int framenum)
{
  ...
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task label(ClearParticlesMT) in(dependency_past[ii]) out(z[i]) 
      ClearParticlesMT(i);
  }
  //#pragma omp taskwait
  int* y = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
       #pragma omp task label (RebuildGridMT) in( { z[ neighborhood[i][j] ], j=0:neighborhood[i].size() } ) out( y[i] ) 
      RebuildGridMT(i);
  }
  //#pragma omp taskwait
  int* x = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task label(InitDensitiesAndForcesMT) in( { y[ neighborhood[i][j] ], j=0:neighborhood[i].size() } ) out( x[i] )
      InitDensitiesAndForcesMT(i);
  }
  //#pragma omp taskwait
  int* a = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task label(ComputeDensitiesMT) in( { x[ neighborhood[i][j] ], j=0:neighborhood[i].size() } ) out( a[i] ) 
      ComputeDensitiesMT(i);
  }
  //#pragma omp taskwait
  int* b = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task label (ComputeDensities2MT) in( { a[ neighborhood[i][j] ], j=0:neighborhood[i].size() } ) out( b[i] ) 
      ComputeDensities2MT(i);
  }
  //#pragma omp taskwait
  int* c = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task label(ComputeForcesMT) in( { b[ neighborhood[i][j] ], j=0:neighborhood[i].size() } ) out( c[i] ) 
      ComputeForcesMT(i);
  }
  //#pragma omp taskwait
  int* d = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task label(ProcessCollisionsMT) in( { c[ neighborhood[i][j] ], j=0:neighborhood[i].size() } ) out( d[i] ) 
      ProcessCollisionsMT(i);
  }
  //#pragma omp taskwait
  int* e = new int[threadnum];
  for(int i = 0; i < threadnum; ++i) {
      #pragma omp task label(AdvanceParticlesMT) in( { d[ neighborhood[i][j] ], j=0:neighborhood[i].size() } ) out( e[i] ) concurrent(e)
...
void *AdvanceFramesMT(int framenum, int threadnum)
{
  ...
  for(int i = 0; i<threadnum; ++i) {
        compute_neighbors(i, neighborhood[i] );
