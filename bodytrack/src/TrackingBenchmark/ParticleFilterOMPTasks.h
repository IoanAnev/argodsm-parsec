...
template<class T> 
class ParticleFilterOMPTasks : public ParticleFilter<T> {
...
template<class T>
void ParticleFilterOMPTasks<T>::CalcWeights(std::vector<Vectorf > &particles)
{
	...
	{	
		...
		#pragma omp task depend(out: _mWeights[j], _valid[j]) depend(in: _particles[j]) firstprivate(j, bsize) //label(CalcLikelihoods)
...
template<class T> 
void ParticleFilterOMPTasks<T>::GenerateNewParticles(int k)
{	int p = 0;
	...
	for(int j=0; j < mNParticles; j+=step) {
		...
		int index = mIndex[j];
		#pragma omp task depend(inout: _mNewParticles[j]) depend(in: _mParticles, _mIndex[j], _mRnd[j]) firstprivate(index, j, k, bsize) //label(GenerateNewParticles)
...
template<class T>
bool ParticleFilterOMPTasks<T>::Update(fpType timeval)								//weights have already been computed from previous step or initialization
{						
	//printf("OMPSSY OMPSS\n");
