...
// 	printf("Bpp=%d\n", Bpp);
	for(int j=0; j < h; j+=step) {
		...
		#pragma omp task out(pdst[index]) in(psrc[index], kernel[0]) firstprivate(index, bsize) label(FlexFilterRowVOMPSS)
...
bool FlexFilterColumnVOMPSS(FlexImage<Im8u,1> &src, FlexImage<Im8u,1> &dst, float *kernel, int kernelSize, bool allocate = true)
{
	...
	for(int j=0; j < h; j+=step) {
		...
		#pragma omp task out(pdst[index]) in(psrc[index], kernel[0]) firstprivate(index, bsize) label(FlexFilterColumnVOMPSS)
...
inline void GradientMagThresholdOMPSS(FlexImage8u &src, FlexImage8u &r, float threshold)
{
	...
	for(int j=1; j < src.Height()-1; j+=step) {
		...
		#pragma omp task out(pr[index]) in(psrc[index]) firstprivate(index, bsize) label(GradientMagThresholdOMPSS)
