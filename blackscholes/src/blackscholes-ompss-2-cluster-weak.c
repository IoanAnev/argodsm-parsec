...
void bs_thread(fptype *prices) {
	int generic_chunk_numOptions = numOptions / nanos6_get_num_cluster_nodes();

	for (int j = 0; j < NUM_RUNS; j++) {
		for (int z = 0; z < numOptions; z += generic_chunk_numOptions) {
			int node_id, chunk_per_node_numOptions;
			node_chunk(&node_id, &chunk_per_node_numOptions, numOptions, z, generic_chunk_numOptions);

			#pragma oss task weakin( sptprice  [z;chunk_per_node_numOptions],	\
						 strike    [z;chunk_per_node_numOptions],	\
						 rate      [z;chunk_per_node_numOptions],	\
						 volatility[z;chunk_per_node_numOptions],	\
						 otime     [z;chunk_per_node_numOptions],	\
						 otype     [z;chunk_per_node_numOptions])	\
					 weakout(prices    [z;chunk_per_node_numOptions])	\
					 firstprivate(z, chunk_per_node_numOptions, BSIZE)	\
					 node(node_id)						\
					 label("remote: BlkSchlsEqEuroNoDiv")
			{
				#pragma oss task in( sptprice  [z;chunk_per_node_numOptions],	\
						     strike    [z;chunk_per_node_numOptions],	\
						     rate      [z;chunk_per_node_numOptions],	\
						     volatility[z;chunk_per_node_numOptions],	\
						     otime     [z;chunk_per_node_numOptions],	\
						     otype     [z;chunk_per_node_numOptions])	\
						 out(prices    [z;chunk_per_node_numOptions])	\
				...
				for (int i = z; i < z+chunk_per_node_numOptions; i += BSIZE) {
					int beg, end, chunk;
					task_chunk(&beg, &end, &chunk, z+chunk_per_node_numOptions, i, BSIZE);
...
#define LINESIZE 64
	...
	for (int z = 0; z < numOptions; z += generic_chunk_numOptions) {
		...
		#pragma oss task weakin( data      [z;chunk_per_node_numOptions])	\
				 weakout(otype     [z;chunk_per_node_numOptions],	\
				         sptprice  [z;chunk_per_node_numOptions],	\
				         strike    [z;chunk_per_node_numOptions],	\
				         rate      [z;chunk_per_node_numOptions],	\
				         volatility[z;chunk_per_node_numOptions],	\
				         otime     [z;chunk_per_node_numOptions])	\
		...
		{
			#pragma oss task in( data      [z;chunk_per_node_numOptions])	\
					 out(otype     [z;chunk_per_node_numOptions],	\
					     sptprice  [z;chunk_per_node_numOptions],	\
					     strike    [z;chunk_per_node_numOptions],	\
					     rate      [z;chunk_per_node_numOptions],	\
					     volatility[z;chunk_per_node_numOptions], 	\
