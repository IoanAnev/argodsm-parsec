...
#define OMPSS_LOCK_INIT(l) omp_init_lock(l)
#define OMPSS_LOCK_DESTROY(l) omp_destroy_lock(l)
#define OMPSS_LOCK(l) omp_set_lock(l)
#define OMPSS_UNLOCK(l) omp_unset_lock(l)
...
int mbuffer_system_init() {
  ...
  assert(locks==NULL);
  locks = malloc(NUMBER_OF_LOCKS * sizeof(omp_lock_t));
  if(locks==NULL) return -1;
  for(i=0; i<NUMBER_OF_LOCKS; i++) {
		OMPSS_LOCK_INIT(&locks[i]);
	}
#endif
...
#elif defined(ENABLE_OMPSS) || defined(ENABLE_OMP)
	OMPSS_LOCK(&locks[lock_hash(m->mcb)]);
  assert(m->mcb->i>=1);
  m->mcb->i++;
...
#endif
// #ifdef ENABLE_OMPSS
//   #pragma omp critical(mbuffer_lock)
//   {
// #endif
  //Update reference counter
#if defined(ENABLE_PTHREADS)
...
#elif defined(ENABLE_OMPSS) || defined(ENABLE_OMP)
	OMPSS_LOCK(&locks[lock_hash(m1->mcb)]);
  assert(m1->mcb->i>=1);
  m1->mcb->i++;
  OMPSS_UNLOCK(&locks[lock_hash(m1->mcb)]);
#else
  assert(m1->mcb->i>=1);
  m1->mcb->i++;
#endif //ENABLE_PTHREADS
