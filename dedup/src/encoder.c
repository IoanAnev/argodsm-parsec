...
#define CHUNK_MAX_NUM (500)
#define RFCHUNK_MAX_NUM (500000)
#ifdef ENABLE_STATISTICS//Keep track of block granularity with 2^CHUNK_GRANULARITY_POW resolution (for statistics)
...
//   unsigned int CChunks;
//   unsigned int RFChunks;
...
#if defined(ENABLE_OMPSS) || defined(ENABLE_OMP)
void WriteOutput(int *fd, chunk_t ** chunks);
#endif
...
static void print_stats(stats_t *s) {
  ...
  printf("Total chunks:                  %14.2d chunks\n", nTotalChunks);
...
#if defined(ENABLE_OMPSS) || defined(ENABLE_OMP)
void Compress(chunk_t * chunk) {
	//printf("Compressing %p\n", chunk);
  int r;
  if(chunk->header.isDuplicate) {
		//printf("Duplcate Compressing exit\n");
		return;
  }
#ifdef ENABLE_STATISTICS
  //stats_t *thread_stats = malloc(sizeof(stats_t));
...
void Compress_bulk(chunk_t ** chunk, int bulk_size) {
	int i;
...
#endif
#if defined(ENABLE_OMPSS) || defined(ENABLE_OMP)
  //#pragma omp critical(hashtable_lock)
  //{
 ...
 */
#if defined(ENABLE_OMPSS) || defined(ENABLE_OMP)
void Deduplicate(chunk_t * chunk) {
//printf("Deduplicating %p\n", chunk);
  int r;
...
#endif //ENABLE_STATISTICS
//printf("Done Deduplicating\n");
}

void Deduplicate_bulk(chunk_t ** chunk, int bulk_size) {
	printf("running bulk with size %d\n", bulk_size);
	int i;
	for(i=0; i < bulk_size; i++) {
		Deduplicate(chunk[i]);
	}
...
#if defined(ENABLE_OMPSS) || defined(ENABLE_OMP)
void FragmentRefine(chunk_t **chunks) {

  int nRFChunks = 0;
	int bulk_size = 0;
  chunk_t *chunk = chunks[0]; 
  //chunks[0] = (chunk_t**)calloc(CHUNK_MAX_NUM, sizeof(chunk_t*));
  int r = 0;
  chunk_t *temp;
  u32int * rabintab = malloc(256*sizeof rabintab[0]);
  u32int * rabinwintab = malloc(256*sizeof rabintab[0]);
  ...
  do {
    ...
    if(offset < chunk->uncompressed_data.n) {
      ...
      chunks[nRFChunks] = chunk;
			Deduplicate(chunks[nRFChunks]);
			Compress(chunks[nRFChunks]);
			//bulk_size++;
// 			bulk_size++;
// 			if(bulk_size == PIPELINE_WU) {
// 				printf("Bulk out -> nRFChunks:%d, bulk_size:%d\n", nRFChunks, bulk_size);
// 				assert(chunks[nRFChunks-1]);
// 				#pragma omp task inout(chunks[nRFChunks;bulk_size]) firstprivate(nRFChunks, bulk_size)
// 				Deduplicate_bulk(&chunks[nRFChunks], bulk_size);
// 				#pragma omp task inout(chunks[nRFChunks;bulk_size]) firstprivate(nRFChunks, bulk_size)
// 				Compress_bulk(&chunks[nRFChunks], bulk_size);
// 				bulk_size = 0;
// 			}
			nRFChunks++;
			//#pragma omp taskwait
      ...
      chunks[nRFChunks] = chunk;
			...
			Compress(chunks[nRFChunks]);
// 			bulk_size++;
// 			printf("else Bulk out -> nRFChunks:%d, bulk_size:%d\n", nRFChunks, bulk_size);
// 			#pragma omp task inout(chunks[nRFChunks;bulk_size]) firstprivate(nRFChunks, bulk_size)
// 			Deduplicate_bulk(&chunks[nRFChunks], bulk_size);
// 			#pragma omp task inout(chunks[nRFChunks;bulk_size]) firstprivate(nRFChunks, bulk_size)
// 			Compress_bulk(&chunks[nRFChunks], bulk_size);
...
void Fragment(struct thread_args *args){
	...
	#pragma omp parallel
	{
		#pragma omp single 
		{
			...
			size_t preloading_buffer_seek = 0;
			int fd = args->fd;
			int fdout = create_output_file(conf->outfile);
			int *fp	 = &fdout;
			int last_bulk = 0;
			int i;
			int nCChunks = 0;
			chunk_t ***chunks = (chunk_t***)calloc(CHUNK_MAX_NUM, sizeof(chunk_t**));
			for(i=0; i < CHUNK_MAX_NUM; i++)
				chunks[i] = (chunk_t**)calloc(RFCHUNK_MAX_NUM, sizeof(chunk_t*));
			//ringbuffer_t send_buf;
			sequence_number_t anchorcount = 0;
			int r;
...
#ifdef ENABLE_OMP
				  #pragma omp task depend(inout: chunks[nCChunks]) firstprivate(nCChunks) //label(Compress)
#else
		...
		// 			Compress(chunks[nCChunks]);
#ifdef ENABLE_OMP
					#pragma omp task depend(inout: fp) depend(in: chunks[nCChunks]) //label(WriteOutput)					
#else
		...
		// 	for(i=nCChunks; i < CHUNK_MAX_NUM; i++) {
		// 		if(chunks[i] == NULL) continue;
		// 		#pragma omp taskwait on(chunks[i])
		// 		#pragma omp task inout(fp) in(chunks[i]) label(WriteOutput)
		// 		write_chunk_to_file(fd, chunks[i]);
		// 	}
			#pragma omp taskwait
			//printf("nCChunks=%d\n", nCChunks);
			//WriteOutput(chunks);
		// 	//freeChunks(fp, chunks);
...
void freeChunks(int *fd, chunk_t *** chunks) {
		int i, j;
	  for(i = 0; i < CHUNK_MAX_NUM; i++) {
			if(!chunks[i*CHUNK_MAX_NUM]) break;
			for(j=0; j < RFCHUNK_MAX_NUM; j++)	{
			chunk_t *chunk = chunks[i*CHUNK_MAX_NUM+j];
      if(!chunk) break;
      if(chunk->header.isDuplicate) {
				free(chunk);
				chunk=NULL;
...
void Encode(config_t * _conf) {
  ...
  __parsec_roi_begin();
#endif
  printf("running ompss version\n");
  //first pipeline stage (input)
  Fragment(&generic_args);
#ifdef ENABLE_PARSEC_HOOKS
  __parsec_roi_end();
#endif
