...
extrae_type_t EXTRAE_MANUAL = 10000;
//extrae_type_t EXTRAE_PARSEC_BARRIER = 1000;
...
static BTimes barrier_times;

int timed_pthread_barrier_wait(pthread_barrier_t* barrier, int pid) {
  double barrier_start = getusec_();
#ifdef ENABLE_THREADS
  pthread_barrier_wait(barrier);
#endif
  barrier_times.barrier_time[pid*barrier_times.stride] += (getusec_()-barrier_start);
}
...
void extrae_register_event_types() {
  ...
  char description1[] = "parsec-barrier";
  char description2[] = "pkmedian-hiz";
  char description3[] = "pgain-open-center";
  char description4[] = "pgain-save-money";
  char description5[] = "pspeedy-open-center";
  char description6[] = "pspeedy-first-center";
  char description7[] = "pkmedian";
  char description8[] = "pspeedy";
  char description9[] = "pgain";
  char description10[] = "contcenters";
  char description11[] = "copycenters";
  char description12[] = "selectfeasible-fast";
  char description13[] = "pgain-updt-costs";
  char description14[] = "pgain-center-tab";
  char description15[] = "pgain-center-tab-update";
  ...
  extrae_descriptions[3] = (char*)calloc(strlen(description3)+1,sizeof(char));
  extrae_descriptions[4] = (char*)calloc(strlen(description4)+1,sizeof(char));
  extrae_descriptions[5] = (char*)calloc(strlen(description5)+1,sizeof(char));
  extrae_descriptions[6] = (char*)calloc(strlen(description6)+1,sizeof(char));
  extrae_descriptions[7] = (char*)calloc(strlen(description7)+1,sizeof(char));
  extrae_descriptions[8] = (char*)calloc(strlen(description8)+1,sizeof(char));
  extrae_descriptions[9] = (char*)calloc(strlen(description9)+1,sizeof(char));
  extrae_descriptions[10] = (char*)calloc(strlen(description10)+1,sizeof(char));
  extrae_descriptions[11] = (char*)calloc(strlen(description11)+1,sizeof(char));
  extrae_descriptions[12] = (char*)calloc(strlen(description12)+1,sizeof(char));
  extrae_descriptions[13] = (char*)calloc(strlen(description13)+1,sizeof(char));
  extrae_descriptions[14] = (char*)calloc(strlen(description14)+1,sizeof(char));
  extrae_descriptions[15] = (char*)calloc(strlen(description15)+1,sizeof(char));
  ...
  strncpy(extrae_descriptions[3],description3,strlen(description3)+1);
  strncpy(extrae_descriptions[4],description4,strlen(description4)+1);
  strncpy(extrae_descriptions[5],description5,strlen(description5)+1);
  strncpy(extrae_descriptions[6],description6,strlen(description6)+1);
  strncpy(extrae_descriptions[7],description7,strlen(description7)+1);
  strncpy(extrae_descriptions[8],description8,strlen(description8)+1);
  strncpy(extrae_descriptions[9],description9,strlen(description9)+1);
  strncpy(extrae_descriptions[10],description10,strlen(description10)+1);
  strncpy(extrae_descriptions[11],description11,strlen(description11)+1);
  strncpy(extrae_descriptions[12],description12,strlen(description12)+1);
  strncpy(extrae_descriptions[13],description13,strlen(description13)+1);
  strncpy(extrae_descriptions[14],description14,strlen(description14)+1);
  strncpy(extrae_descriptions[15],description15,strlen(description15)+1);
  char et_description[] = "extrae_manual_events";
  ...
  Extrae_define_event_type(&EXTRAE_MANUAL,event_type_description,&extrae_nvalues,extrae_values,extrae_descriptions); 
...
#ifdef ENABLE_EXTRAE
    //Extrae_user_function(1);
    Extrae_eventandcounters(EXTRAE_MANUAL,8);
#endif
#ifdef ENABLE_THREADS
 pthread_barrier_wait(barrier);
#endif
  ...
  /* create center at first point, send it to itself */
#ifdef ENABLE_EXTRAE
  Extrae_eventandcounters(EXTRAE_MANUAL,6);
#endif
  for( int k = k1; k < k2; k++ )    {
    float distance = dist(points->p[k],points->p[0],points->dim);
    points->p[k].cost = distance * points->p[k].weight;
...
#ifdef ENABLE_EXTRAE
  Extrae_eventandcounters(EXTRAE_MANUAL,0);
#endif
...
#ifdef ENABLE_EXTRAE
  Extrae_eventandcounters(EXTRAE_MANUAL,5);
#endif
      for( int k = k1; k < k2; k++ )
      {
        float distance = dist(points->p[i],points->p[k],points->dim);
...
double pgain(long x, Points *points, double z, long int *numcenters, int pid, pthread_barrier_t* barrier)
{
#ifdef ENABLE_EXTRAE
    //Extrae_user_function(1);
  Extrae_eventandcounters(EXTRAE_MANUAL,9);
#endif
...
#ifdef ENABLE_EXTRAE
  Extrae_eventandcounters(EXTRAE_MANUAL,14);
#endif
  /*For each center, we have a *lower* field that indicates 
    how much we will save by closing the center. 
    Each thread has its own copy of the *lower* fields as an array.
...
#ifdef ENABLE_EXTRAE
  Extrae_eventandcounters(EXTRAE_MANUAL,15);
#endif
  for( int i = k1; i < k2; i++ ) {
    if( is_center[i] ) {
      center_table[i] += (int)work_mem[pid*stride];
...
#ifdef ENABLE_EXTRAE
  Extrae_eventandcounters(EXTRAE_MANUAL,13);
#endif
  //my *lower* fields
  double* lower = &work_mem[pid*stride];
  //global *lower* fields
...
#ifdef ENABLE_EXTRAE
  Extrae_eventandcounters(EXTRAE_MANUAL,3);
#endif
  for ( int i = k1; i < k2; i++ ) {
    if( is_center[i] ) {
      double low = z;
 ...
 pthread_barrier_wait(barrier);
  ...
  if ( gl_cost_of_opening_x < 0 ) {
    //  we'd save money by opening x; we'll do it
#ifdef ENABLE_EXTRAE
  Extrae_eventandcounters(EXTRAE_MANUAL,4);
#endif
    for ( int i = k1; i < k2; i++ ) {
      bool close_center = gl_lower[center_table[points->p[i].assign]] > 0 ;
      if ( switch_membership[i] || close_center ) {
...
#ifdef ENABLE_EXTRAE
    //Extrae_user_function(1);
  Extrae_eventandcounters(EXTRAE_MANUAL,12);
#endif
	       ...
	       int pid, pthread_barrier_t* barrier )
{
#ifdef ENABLE_EXTRAE
    //Extrae_user_function(1);
    Extrae_eventandcounters(EXTRAE_MANUAL,7);
#endif
  int i;
  double cost;
  double lastcost;
 ...
 pthread_barrier_wait(barrier);
  ...
  double myhiz = 0;
#ifdef ENABLE_EXTRAE
  Extrae_eventandcounters(EXTRAE_MANUAL,2);
#endif
  ...
  Extrae_eventandcounters(EXTRAE_MANUAL,0);
#endif
  //barrier_times.myhiz_time[pid] += (getusec_() - myhiz_start);
  //barrier_times.myhiz_tasks[pid]++;
...
#ifdef ENABLE_EXTRAE
    //Extrae_user_function(1);
  Extrae_eventandcounters(EXTRAE_MANUAL,10);
#endif
  long i, ii;
  float relweight;
...
#ifdef ENABLE_EXTRAE
    //Extrae_user_function(1);
  Extrae_eventandcounters(EXTRAE_MANUAL,11);
#endif
  long i;
  long k;
...
void localSearch( Points* points, long kmin, long kmax, long* kfinal ) {
    pthread_barrier_t barrier;
    pthread_t* threads = (pthread_t *)calloc(nproc,sizeof(pthread_t)); //new pthread_t[nproc];
    pkmedian_arg_t* arg = (pkmedian_arg_t *)calloc(nproc,sizeof(pkmedian_arg_t)); //new pkmedian_arg_t[nproc];
...
int main(int argc, char **argv)
{
  ...
  int dim;
  /*barrier_times.stride = CACHE_LINE/sizeof(double);
  barrier_times.barrier_time = (double *)calloc(atoi(argv[9])*barrier_times.stride,sizeof(double));
  barrier_times.size = atoi(argv[9]);

  barrier_times.myhiz_time = (double *)calloc(atoi(argv[9]),sizeof(double));
  barrier_times.myhiz_tasks = (int *)calloc(atoi(argv[9]),sizeof(int));*/
#ifdef ENABLE_EXTRAE
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  printf("\n");
  /*for (int i = 0; i < nproc; i++) {
    fprintf(stdout,"BARRIER TIME THREAD %d:\t %f",i,barrier_times.barrier_time[i*barrier_times.stride]/1e6);
    printf("\n");
    fprintf(stdout,"AVG MYHIZ TIME THREAD %d:\t %f",i,barrier_times.myhiz_time[i]/barrier_times.myhiz_tasks[i]);
    printf("\n");
  }*/
  /*free(barrier_times.barrier_time);
  free(barrier_times.myhiz_time);
  free(barrier_times.myhiz_tasks);*/
