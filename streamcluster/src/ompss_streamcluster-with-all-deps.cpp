...
float pspeedy(Points *points, float z, long *kcenter)
{
  ...
  Point* po = points->p;
  int dim = points->dim;
  float* block = po[0].coord;
  /* create center at first point, send it to itself */
for (int p = 0; p < nproc; p++) {
k1 = bsize*p; k2 = k1 + bsize; if (p == nproc-1) k2 = points->num;
      //nanos_current_socket(p%NUMANODES);
  //those tasks only modify the cost and the assign of the points. The coordinates are being consumed
  #pragma omp task in(block[k1*dim:k2*dim],po[0],block[0:dim]) inout(po[k1:k2]) firstprivate(k1,k2) label(PSPEEDY-FIRST-CENTER)
  {
    for( int k = k1; k < k2; k++ )    {
  ...
  *kcenter = 1;
   ...
   // I am the master thread. I decide whether to open a center and notify others if so.
    ...
    for(i = 1; i < points->num; i++ )  {
      ...
      //nanos_current_socket(p%NUMANODES);
          printf("k1 is %i, k2 is %i, dim is %i\n", k1, k2, dim);
          #pragma omp task in(block[k1*dim:k2*dim],po[i],block[i*dim:i*dim+dim]) inout(po[k1:k2]) firstprivate(k1,k2) label(0)
          {
	        for( int k = k1; k < k2; k++ )  {
...
for (int p = 0; p < nproc; p++) {
  ...
  //#pragma omp target device(smp) copy_deps(ended)
      //nanos_current_socket(p%NUMANODES);
  #pragma omp task in(po[k1:k2],block[k1*dim:k2*dim],po[x],block[x*dim:x*dim+dim]) concurrent([count]gl_lower,gl_cost_of_opening_x) \
   inout(switch_membership[k1:k2]) \
   firstprivate(k1,k2,p,count) label(PGAIN-UPDT-COSTS)
  {
    ...
    for (int i = k1; i < k2; i++ ) {
      ...
      if ( x_cost < current_cost ) {
        ...
        //if switch_membership changes, gl_cost_of_opening also change. With input swmembership
...
gl_cost_of_opening_x += z;
  ...
  if ( gl_cost_of_opening_x < 0 ) {
    ...
    for (int p = 0; p < nproc; p++) {
      k1 = bsize*p; k2 = k1 + bsize; if (p == nproc-1) k2 = points->num;
//      nanos_current_socket(p%NUMANODES);
      #pragma omp task inout(po[k1:k2],is_center[k1:k2]) in(block[k1*dim:k2*dim],po[x],block[x*dim:x*dim+dim],switch_membership[k1:k2],[count]gl_lower) label(PGAIN-SAVE-MONEY)
      {
        for ( int i = k1; i < k2; i++ ) {
...
//      nanos_current_socket(p%NUMANODES);
    #pragma omp task in(block[k1*dim:k2*dim],block[0:dim],po[k1:k2],po[0]) concurrent(hiz) firstprivate(k1,k2) label(PKMEDIAN-HIZ)
    {
    double myhiz = 0;
    for (long kk=k1;kk < k2; kk++ ) {
      myhiz += dist(points->p[kk], points->p[0],
	  	      dim)*points->p[kk].weight;
    }
    #pragma omp atomic
    hiz += myhiz;
    }
  ...
  if (points->num <= kmax) {
    ...
    for (int p = 0; p < nproc; p++) {
      ...
      if (p == nproc-1) k2 = points->num;
//      nanos_current_socket(p%NUMANODES);
      #pragma omp task in(block[k1*dim:k2*dim]) inout(po[k1:k2]) firstprivate(k1,k2) label(PKMEDIAN-ALL-FL)
      {
        for (long kk=k1;kk<k2;kk++) {
