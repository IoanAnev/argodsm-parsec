...
float pspeedy(Points *points, float z, long *kcenter)
{
	...
	for (int p = 0; p < nproc; p++) {
		...
		#pragma oss task inout(po[k1:k2]) firstprivate(k1,k2) label("PSPEEDY-FIRST-CENTER")
		{
			for( int k = k1; k < k2; k++ )    {
	...
	*kcenter = 1;
	...
	for(i = 1; i < points->num; i++ )  {
		...
		if( to_open )  {
			...
			for (int p = 0; p < nproc; p++) {
				...
				// nanos_current_socket(p%NUMANODES);
				#pragma oss task inout(po[k1:k2]) firstprivate(k1,k2) label("PSPEEDY-OPEN-CENTERS")
				{
					for( int k = k1; k < k2; k++ )  {
	...
	for (int p = 0; p < nproc; p++) {
		...
		//#pragma oss task inout(po[k1:k2]) firstprivate(k1,k2,mytotal,p) label("PSPEEDY-COST-REDUCTION")
		//{ 
		mytotal = 0;
		for( int k = k1; k < k2; k++ )  {
...
double pgain(long x, Points *points, double z, long int *numcenters)
{
	...
	for (int p = 0; p < nproc; p++) {
		k1 = bsize*p; k2 = k1 + bsize; if (p == nproc-1) k2 = points->num;
		//#pragma oss target device(smp) copy_deps(ended)
		// nanos_current_socket(p%NUMANODES);
		#pragma oss task concurrent([num_points]po,[count]gl_lower) \
	...
	if ( gl_cost_of_opening_x < 0 ) {
		...
		for (int p = 0; p < nproc; p++) {
			k1 = bsize*p; k2 = k1 + bsize; if (p == nproc-1) k2 = points->num;
			// nanos_current_socket(p%NUMANODES);
			#pragma oss task inout(po[k1:k2]) in(switch_membership[k1:k2],[count]gl_lower) label("PGAIN-SAVE-MONEY")
			{
				for ( int i = k1; i < k2; i++ ) {
...
float pkmedian(Points *points, long kmin, long kmax, long* kfinal)
{
	...
	for (int p = 0; p < nproc; p++) {
		...
		// nanos_current_socket(p%NUMANODES);
		#pragma oss task in(po[k1:k2]) concurrent(hiz) firstprivate(k1,k2) label("PKMEDIAN-HIZ")
		{
			double myhiz = 0;
	...
	if (points->num <= kmax) {
		...
		for (int p = 0; p < nproc; p++) {
			...
			if (p == nproc-1) k2 = points->num;
			// nanos_current_socket(p%NUMANODES);
			#pragma oss task inout(po[k1:k2]) firstprivate(k1,k2) label("PKMEDIAN-ALL-FL")
			{
				for (long kk=k1;kk<k2;kk++) {
		...
		long chunksize, long centersize, char* outfile )
{
	...
	float* block = (float*)nanos6_dmalloc( chunksize*dim*sizeof(float), nanos6_equpart_distribution, 0, NULL);
	float* centerBlock = (float*)nanos6_dmalloc(centersize*dim*sizeof(float), nanos6_equpart_distribution, 0, NULL);
	long* centerIDs = (long*)nanos6_lmalloc(centersize*dim*sizeof(long));
	...
	while(1) {
		...
		#pragma oss task out(is_center[0;points.num]) label("calloc")
		for (long i = 0; i < points.num; ++i) { is_center[i] = 0; }
		#pragma oss taskwait
		center_table = (int*)nanos6_dmalloc(points.num*sizeof(int), nanos6_equpart_distribution, 0, NULL);
		gl_lower = (double*)nanos6_dmalloc(points.num*sizeof(double), nanos6_equpart_distribution, 0, NULL);
		...
		nanos6_dfree(is_center, points.num*sizeof(bool));
		nanos6_dfree(switch_membership, points.num*sizeof(bool));
		nanos6_dfree(center_table, points.num*sizeof(int));
		nanos6_dfree(gl_lower, points.num*sizeof(double));
	...
	switch_membership = (bool*)nanos6_dmalloc(centers.num*sizeof(bool), nanos6_equpart_distribution, 0, NULL);
	is_center = (bool*)nanos6_dmalloc(centers.num*sizeof(bool), nanos6_equpart_distribution, 0, NULL);
	#pragma oss task out(is_center[0;centers.num]) label("calloc")
	for (long i = 0; i < centers.num; ++i) { is_center[i] = 0; }
	#pragma oss taskwait
	center_table = (int*)nanos6_dmalloc(centers.num*sizeof(int), nanos6_equpart_distribution, 0, NULL);
	gl_lower = (double*)nanos6_dmalloc(centers.num*sizeof(double*), nanos6_equpart_distribution, 0, NULL);
	...
	nanos6_dfree(is_center, centers.num*sizeof(bool));
	nanos6_dfree(switch_membership, centers.num*sizeof(bool));
	nanos6_dfree(center_table, centers.num*sizeof(int));
	nanos6_dfree(gl_lower, centers.num*sizeof(double*));

	nanos6_dfree(block, chunksize*dim*sizeof(float));
	nanos6_dfree(centerBlock, centersize*dim*sizeof(float));
	
	nanos6_lfree(centerIDs, centersize*dim*sizeof(long));
	nanos6_lfree(points.p, chunksize*sizeof(Point));
	nanos6_lfree(centers.p, centersize*sizeof(Point));
}
