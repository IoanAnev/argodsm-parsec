...
//#pragma omp task inout([bsize] p) in([1] ref, dim, bsize)
void CenterCreate(Point* p, Point* ref, int dim, int bsize) {
    
	 for( int k = 0; k < bsize; k++ )    {
		float distance = dist(p[k], *ref, dim);
		p[k].cost = distance * p[k].weight;
		p[k].assign=0;
	 }  
}

//#pragma omp task inout([bsize] p) concurrent(*total_cost)
void CenterOpen(Point *p, Point ref, int refindex, int dim, int bsize, double* total_cost, int type /*type=0: compute. type=1: reduction */) {

	if(type) {
		double local_total = 0.0;
		for(int k = 0; k < bsize; k++ )  
			local_total += p[k].cost;
		//#pragma omp atomic
		*total_cost += local_total;
	}
	else {
	  for(int k = 0; k < bsize; k++ )  {
		float distance = dist(ref, p[k], dim);
			if( refindex && distance*p[k].weight < p[k].cost )  {
				p[k].cost = distance * p[k].weight;
				p[k].assign = refindex;
			}
		}
...
float pspeedy(Points *points, float z, long *kcenter)
{
  ...
  /* create center at first point, send it to itself */
	...
	Point* p = &(points->p[0]);
	int dim = points->dim;
	for(int k=0; k < points->num; k += grain_size) {
		//CenterCreate(&(points->p[k]), &(points->p[0]), points->dim, grain_size);
		
		#pragma omp task out(p[ k ; k+grain_size ]) firstprivate(k, grain_size)
		{
			for( int ii = k; ii < k+grain_size; ii++ )    {
				float distance = dist(p[ii], p[0], dim);
				p[ii].cost = distance * p[ii].weight;
				p[ii].assign=0;
			}
		}
	}
	#pragma omp taskwait
   double local_costs[NUM_DIVISIONS];
   *kcenter = 1;
    ...
    for(i = 1; i < points->num; i++ )  {
      ...
      if( to_open )  {
		...
		for(int k=0; k < points->num; k += grain_size) {
			CenterOpen(&(points->p[k]), points->p[i], i, points->dim, grain_size, &totalcost, 0);
		}
      }

    /* One last time for actual reduction */
	for(int k=0; k < points->num; k += grain_size) {
		CenterOpen(&(points->p[k]), points->p[0], 0, points->dim, grain_size, &totalcost, 1);
	}


    totalcost =z*(*kcenter);
    //for( int i = 0; i < NUM_DIVISIONS; i++ ) {
    //    totalcost += local_costs[i];
    //}
  }
  return(totalcost);
}
...
double pgain(long x, Points *points, double z, long int *numcenters, int pid)
{
  ...
  //my block
  int nproc1 = 1;
  long bsize = points->num/nproc1;
  long k1 = bsize * pid;
  long k2 = k1 + bsize;
  if( pid == nproc1-1 ) k2 = points->num;
  ...
  if( pid==0 )    { 
    work_mem = (double*) malloc(stride*(nproc1+1)*sizeof(double));
  ...
  */
  ...
  if( pid== 0 ) memset(work_mem+nproc1*stride,0,stride*sizeof(double));

  //my *lower* fields
  double* lower = &work_mem[pid*stride];
  //global *lower* fields
  double* gl_lower = &work_mem[nproc1*stride];
  ...
  if( pid==0 ) {
    ...
    //aggregate
    for( int p = 0; p < nproc1; p++ ) {
	  /*if((int)work_mem[p*stride + K] < 0) {
		fprintf(stderr, "g_number_of_centers_to_close2=%d\n", (int)work_mem[p*stride + K]);
		fprintf(stderr, "p=%d, stride=%d, K=%d\n", p, stride, K);
	  }*/
	  ...
	  double z, long *k, double cost, long iter, double e)
{
  ...
  while (change/cost > 1.0*e) {
    ...
    for (i=0;i<iter;i++) {
      ...
      change += pgain(feasible[x], points, z , k, 0);
...
//#pragma omp task concurrent(*hiz) in([bsize] p)
void HizReduction(Point *p, Point ref, int dim, int bsize, double *hiz) {
	double myhiz = 0;
	long ptDimension = dim;

	for(int kk=0; kk < bsize; kk++) {
	  myhiz += dist(p[kk], ref, ptDimension)*p[kk].weight;
	}
	//#pragma omp atomic
	*hiz += myhiz;
}
...
float pkmedian(Points *points, long kmin, long kmax, long* kfinal)
{
  ...
  if(grain_size==0)
	...
	double local_hiz[NUM_DIVISIONS];
	Point *p = &(points->p[0]);
	Point ref = points->p[0];
	//double *hiz_ptr = &hiz;
	for(int kk=0; kk < points->num; kk += grain_size) {
		//#pragma omp task inout(local_hiz[kk/grain_size]) in(p[kk:kk+grain_size], ref, ptDimension, grain_size) firstprivate(kk, grain_size)
		//HizReduction(&p[kk], ref, ptDimension, grain_size, &local_hiz[kk/grain_size]);
		//#pragma omp taskwait
		//#pragma omp task concurrent(hiz) 
		{
			double myhiz = 0;

			for(int ii=kk; ii < kk+grain_size; ii++) {
			  myhiz += dist(p[ii], ref, ptDimension)*p[ii].weight;
			}
			//#pragma omp atomic
			hiz += myhiz;
		}
