...
#pragma omp task label(read_file) private(r) firstprivate(data, total_queries) \
                 inout(loaded_images[total_queries], centinel[read_resource]) shared (loaded_images) if(1) 
{
...
//JL: end
	...
	#pragma omp task in(loaded_images[total_queries]) out(segmented_images[total_queries]) firstprivate(total_queries) label(t_seg)
	segmented_images[total_queries] = t_seg(loaded_images[total_queries]);
	#pragma omp task in(segmented_images[total_queries]) out(extracted_data[total_queries]) firstprivate(total_queries) label(t_extract)
	extracted_data[total_queries] = t_extract(segmented_images[total_queries]);
	#pragma omp task in(extracted_data[total_queries]) out(vectorized_data[total_queries]) firstprivate(total_queries) label(t_vec)
	vectorized_data[total_queries] = t_vec(extracted_data[total_queries]);
	#pragma omp task in(vectorized_data[total_queries]) out(ranked_results[total_queries]) firstprivate(total_queries) label(t_rank)
	ranked_results[total_queries] = t_rank(vectorized_data[total_queries]);
	#pragma omp task in(ranked_results[total_queries]) inout(fout) firstprivate(total_queries) priority (10) label(t_out)
