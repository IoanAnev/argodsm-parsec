...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
	...
	init_time = tv_init.tv_sec+1e-6*tv_init.tv_usec;
	end_time = tv_end.tv_sec+1e-6*tv_end.tv_usec;

	fprintf(stdout, "Execution Time = %.4f sec\n", end_time - init_time);
...
#if defined(HAVE_PTHREAD) || defined(HAVE_OMPSS)
...
#endif
		if(res)
            break;	//frame not readed properly, exiting
